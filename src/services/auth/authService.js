...
export class AuthService {
  ...
  async register(userData) {
    ...
    try {
      ...
      await this.userProfileRepository.createForUser(user.id, {}, { transaction })

      await transaction.commit()

      await emailService.sendActivationEmail(user.email, user.activation_code)
  ...
  async login(emailOrUsername, password) {
    ...
    if (user.status === 0) {
      throw new AppError('Please activate your account first', HTTP_STATUS.UNAUTHORIZED)
    }
  ...
  async activateAccount(activationCode) {
    const user = await this.userRepository.findByActivationCode(activationCode)
    ...
    if (user.status === 1) {
      throw new AppError('Account already activated', HTTP_STATUS.BAD_REQUEST)
    }
