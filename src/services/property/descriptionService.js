...
class DescriptionService {
  async updateAmenities(propertyId, userId, characteristicIds) {
    ...
    await CharacteristicRepository.updatePropertyCharacteristics(propertyId, characteristicIds)
  ...
  async updateDescription(propertyId, userId, data, languageId = 1) {
    ...
    await PropertyTranslationRepository.upsertTranslation(propertyId, languageId, translationData)
  ...
  async tryCompleteDescriptionStep(propertyId) {
    const [translation, amenities] = await Promise.all([
      PropertyTranslationRepository.findByPropertyAndLanguage(propertyId, 1),
      CharacteristicRepository.getPropertyCharacteristics(propertyId)
    ])

    const hasWrittenDescription = translation && translation.title && translation.description
