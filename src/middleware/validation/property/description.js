...
const propertyDescriptionSchema = Joi.object({
  ...
  title: Joi.string().min(10).max(128).allow(null, '').optional()    //eshte required per ta bere description = 1
    .messages({
      'string.min': 'Title must be at least 10 characters',
      'string.max': 'Title must not exceed 128 characters'
    }),
  description: Joi.string().min(50).max(5000).allow(null, '').optional()    //eshte required per ta bere description = 1
    .messages({
